...
pub enum TicketNewError {
    ...
    InvalidStatus(String, #[source] status::ParseStatusError),
}
...
impl Ticket {
    pub fn new(title: String, description: String, status: String) -> Result<Self, TicketNewError> {
        ...
        let s = status.clone();
        let sta = status::Status::try_from(status);

        let status = match sta {
            Ok(s) => s,
            Err(e) => return Err(TicketNewError::InvalidStatus(s, e)),
        };
        Ok(Ticket {
            title,
            description,
